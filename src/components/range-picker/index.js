export default class RangePicker {
  ...
  renderCalendar(showDate) {
    ...
    let table = this.prepareTemplate`<div class="rangepicker__calendar">
  ...
  prepareTemplate (string, ...props) {
    return string.reduce((accum, item, index) => {
      return `${accum}${item.trim()}${props[index] || ''}`;
    }, '');
  }
  ...
  remove () {
    this.$element.remove();
    // TODO: Warning! To remove listener  MUST be passes the same properties as in init
...
function createElement (html) {
  ...
  const wrapper = supportsTemplate ? 'template' : 'div';
  const template = document.createElement(wrapper);
